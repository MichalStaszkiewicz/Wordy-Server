    ...
    try {
        ...
        userAchievements.forEach(userAchievement => {
            ...
            if (userAchievement.achievement.type == 'word') {
                ...
                if (totalWords.totalWords >= userAchievement.achievement.goal && userAchievement.achieved == false) {
        ...
        for await (const achievement of userAchievementsAfterUpdate) {
            ...
            await UserAchievementService.update(user.profile, achievement.id, partial);
...
export function validateImage(imageName: string) {
    ...
    const hasValidExtension = allowedExtensions.some(ext => imageName.endsWith(ext));
...
export async function getTotalKnownWords(userId: string): Promise<any> {
    try {
        ...
        let vocabualaryProgress: ActiveCourseProgress[] = await getProgressForVocabulary(userCourses, user) as ActiveCourseProgress[];
...
export async function getProgressForVocabulary(activeCoursesData: UserCourseEntity[], user: UserEntity) {
    try {
        ...
        for await (const topic of topics) {
            let wordCount: number = (await WordService.getWordsWithTopic(topic)).length;
        ...
        for await (const activeCourse of activeCoursesData) {
            ...
            knownWords = await VocabularyWordService.getKnownWords(activeCourse.mode.vocabulary);
            for (const topic of topics) {
                var knownWordsInGivenTopic = (await VocabularyWordService.getVocabularyWordsByIdAndTopic(activeCourse.mode.vocabulary, topic)).length;
            ...
            const newCourseProgress: ActiveCourseProgress = {
                activeCourse: {
                    ...
                    totalProgress: Number(((knownWords.length / totalWordsCount) * 100).toPrecision(2)),
