...
import { ErrorCodes } from '../const/error_codes'; import path from 'path';
import { Readable } from 'typeorm/platform/PlatformTools';
import * as Hapi from '@hapi/hapi';

import { Messages } from '../const/messages';
import { IImageRequest } from '../interfaces/requests/image_request';
import { validateImage } from '../const/common';
...
export class ImageController {
    public static async getAllImages(request: Request, h: ResponseToolkit) {
        ...
        try {
            ...
            const images = files.filter(file => /\.(jpe?g|png|gif)$/i.test(file)).map(file => {
                const name = file;
          ...
          `;
            ...
            console.error(error);
            return h.response('An error occurred while retrieving the images').code(500);
        }
    }
    public static async getImage(request: Request, response: ResponseToolkit) {
        const imagesDirectory = path.join(__dirname, '..', '..', 'images');
        const imageName = request.params.imageName;
        const validate: boolean = validateImage(imageName);
    ...
    public static async uploadImage(request: IImageRequest, response: ResponseToolkit) {
        const payload = request.payload;
        if (!payload || !payload.image || !payload.image._data || !payload.image._data) {
