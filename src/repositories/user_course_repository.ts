...
export class UserCourseRepository {
    ...
    public static async update(userCourse: Partial<UserCourseEntity>, id: number): Promise<any> {
        ...
        return dataSource.getRepository(UserCourseEntity).update({ id: id }, userCourse);
    ...
    public static async getUserCourse(user: UserEntity, course: CourseEntity): Promise<any> {
        return dataSource.getRepository(UserCourseEntity).findOne({
            where: { user: user, interfaceLanguage: user.profile.settings.interfaceLanguage, course: course }, relations: {

                course: true,
                interfaceLanguage: true, lastTopic: true,
                mode: {
                    vocabulary: true,
